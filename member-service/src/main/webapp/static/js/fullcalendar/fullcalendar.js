...
var DayGrid = FC.DayGrid = Grid.extend(DayTableMixin, {
	...
	renderNumberCellHtml: function(date) {
		...
		var thisDay = new Date(date.format());
		var lunarMonth =  CnMonthofDate(thisDay)//农历月
		var lunarDay  = CnDayofDate(thisDay);//农历日
		var lunarYear = CnYearofDate(thisDay);//农历年
		var festival = SolarTerm(thisDay);//节日
		var holiday = getHoliday(thisDay);
	    if(lunarDay=='正月')
	        holiday = '春节'; 
	    if((lunarMonth+lunarDay)=="正月十五"){
	    	holiday = "元宵节";
	    }
	    if(lunarMonth+lunarDay=="五月初五"){
	    	holiday = "端午节";
	    }
	    if(lunarMonth+lunarDay=="八月十五"){
	    	holiday = "中秋节";
	    }
	    if(lunarMonth+lunarDay=="九月初九"){
	    	holiday = "重阳节";
	    }
	    if(lunarMonth+lunarDay=="腊月三十"){
	    	holiday = "除夕";
	    }
		return '' +
			'<td class="' + classes.join(' ') + '" data-date="' + date.format() + '"><span style="float:left">'+lunarDay+'</span><span style="margin-right:35%;">'+holiday+'</span><span style="float:right">'+date.date()+'</span></td>';
	},
...
function getHoliday(DateGL){
	var month = DateGL.getMonth();
	var date = DateGL.getDate();
	var holiday = "";
	if ((month == 0) && (date == 1)) holiday="元旦";
	if ((month == 1) && (date ==14 )) holiday="情人节";
	if ((month == 2) && (date == 8)) holiday="妇女节";
	if ((month == 2) && (date == 12)) holiday="植树节";

	if ((month == 3) && (date == 1)) holiday="愚人节";
	if ((month == 4) && (date == 1)) holiday="劳动节";
	if ((month == 4) && (date == 9)) holiday="母亲节";

	if ((month == 5) && (date == 1))  holiday="儿童节";
	if ((month == 7) && (date == 1))  holiday="建军节";
	
	if ((month == 8) && (date == 10))  holiday="教师节";

	if ((month == 11) && (date == 24))  holiday="平安夜";
	if ((month == 11) && (date == 25))  holiday="圣诞节";
	return holiday;
}